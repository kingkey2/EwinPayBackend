...
public partial class Common : System.Web.UI.Page
{
    ...
    private static PaymentReport GetPaymentByOrderID(string OrderID)
    {
        string SS;
        ...
        DBCmd.Parameters.Add("@OrderID", SqlDbType.Int).Value = OrderID;
        ...
        if (DT != null)
        {
            if (DT.Rows.Count > 0)
            {
                returnValue = DataTableExtensions.ToList<PaymentReport>(DT).ToList().First();
            }
        }
