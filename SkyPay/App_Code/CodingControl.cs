...
using Newtonsoft.Json.Linq;
    ...
    public class CodingControl {
        ...
        public static void UpdateCDNList() {
            string json = GetWebTextContent("https://ip-ranges.amazonaws.com/ip-ranges.json");
            JObject IPListJO = JObject.Parse(json);

            JArray IP_LIST = JArray.Parse(IPListJO["prefixes"].ToString());

            foreach (JObject Jo in IP_LIST) {
                if (Jo["service"].ToString() != "CLOUDFRONT")
                    continue;
                string[] IP = Jo["ip_prefix"].ToString().Split('/');
